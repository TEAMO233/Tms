...
// 转发CRUD操作 - 全部使用POST请求
export const createForward = (data: ForwardForm) =>
  Network.post("/forward/create", data);
export const getForwardList = () => Network.post("/forward/list");
export const updateForward = (data: ForwardForm) =>
...
export const getForwardClientLink = (forwardId: number) =>
  Network.post<ForwardClientLinkResponse>("/forward/subscription/link", {
    forwardId,
  });
export const createForwardSubscription = () =>
  Network.post<ForwardSubscriptionResponse>("/forward/subscription");
...
export const createInbound = (data: any) =>
  Network.post("/inbound/create", data);
export const oneClickInbound = (nodeId: number, sni?: string) =>
  Network.post("/inbound/one-click", { nodeId, sni });
export const getInboundList = () => Network.post("/inbound/list");
export const deleteInbound = (id: number) =>
  Network.post("/inbound/delete", { id });
export const deleteInboundsByNode = (
  nodeId: number,
  relay?: boolean,
  landingId?: number,
) => Network.post("/inbound/delete-by-node", { nodeId, relay, landingId });
export const assignInboundUser = (data: any) =>
  Network.post("/inbound/assign", data);
export const assignAllToUser = (data: any) =>
  Network.post("/inbound/assign-all", data);
// 「我自己用」:把这台机器/这条中转的协议开给当前登录的管理员自己(不限速/不限量/不到期)
export const assignSelf = (data: any) =>
...
export const oneClickRelay = (
  ...
  name?: string,
  sni?: string,
) => Network.post("/inbound/one-click-relay", { nodeId, link, name, sni });
export const createUdpQuicRelay = (data: UdpQuicRelayCreateForm) =>
  Network.post<UdpQuicRelayResult[]>("/inbound/udp-quic-relay", data);
export const testLanding = (nodeId: number, link: string) =>
  Network.post("/landing/test", { nodeId, link });
export const getLandingList = () => Network.post("/landing/list"); // 仅用于中转卡片显示落地名

// 透明中转 / 线路机模式:入口机 nftables DNAT+SNAT 到主服务器端口
export const createTransparentRelay = (data: TransparentRelayForm) =>
  Network.post<TransparentRelay>("/transparent-relay/create", data);
export const createTransparentRelayBatch = (data: TransparentRelayBatchForm) =>
  Network.post<TransparentRelayBatchResult>(
    "/transparent-relay/batch-create",
    data,
  );
export const getTransparentRelayList = () =>
  Network.post<TransparentRelay[]>("/transparent-relay/list");
export const updateTransparentRelay = (data: TransparentRelayForm) =>
  Network.post("/transparent-relay/update", data);
export const deleteTransparentRelay = (id: number) =>
  Network.post("/transparent-relay/delete", { id });
export const pauseTransparentRelay = (id: number) =>
  Network.post("/transparent-relay/pause", { id });
export const resumeTransparentRelay = (id: number) =>
  Network.post("/transparent-relay/resume", { id });
export const getTransparentRelayStatus = (nodeId: number) =>
  Network.post<TransparentRelayStatusResponse>("/transparent-relay/status", {
    nodeId,
  });
export const getTransparentRelaySubscriptionLink = (id: number) =>
  Network.post<{ link: string }>("/transparent-relay/subscription/link", {
    id,
  });
export const createTransparentRelaySubscription = () =>
  Network.post<TransparentRelaySubscriptionResult>(
    "/transparent-relay/subscription",
  );

// 订阅按线路(车友×机器):一个车友的所有订阅线路
export const getUserLines = (userId: number) =>
  Network.post("/inbound/user-lines", { userId });
